...
class APIFeatures {
  ...
  filter() {
    ...
    // Loop through each key in the parsed object
    const regexQuery: Record<string, { $regex: string; $options: string }> = {};
    for (const key in parsedObj) {
      // Check if the value is truthy
      if (parsedObj[key]) {
        // Create a regular expression for the value of the key
        regexQuery[key] = { $regex: parsedObj[key], $options: "i" };
      }
    }
